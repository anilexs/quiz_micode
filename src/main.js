...
function startQuiz(event) {
  ...
  function displayQuestion(index){
    ...
    const submitButton = getSubmiteButton();
  ...
  function displayFinishMessage(){
    ...
    p.innerHTML = `Tu as eu ${score} sur ${Questions.length} point !`
    app.appendChild(h1);
    app.appendChild(p);
  }
...
function showFeedback(isCorrect, correct, answer){
    ...
    function getFeedbackMessage(isCorrect, correct){
      ...
      paragraph.innerText = isCorrect ? "Bravo ! tu as eu la bonne reponse" : `Desole... mais la bonne reponse etait ${correct} `;
      return paragraph
    }
  ...
  function displayNextQuestionButton(callback){
   ...
   function disableAllanswers(){
    const radioInputes = document.querySelectorAll('input[type="radio"]');
    for (const radio of radioInputes){
      radio.disabled = true;
    }
   }